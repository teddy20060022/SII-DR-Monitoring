...
try:
    ...
    sql0 = "select [MJob].[job_id] from DRMON.dbo.[Master_Job_ID] AS [MJob] WHERE [MJob].[DRSystem] = 'CON' AND [MJob].ProcessCode = 'R' \
    ...
    sql = "SELECT * FROM OPENQUERY (CONFINSDR, \'Select [DRSystem] ,[ProcessCode] ,[LogDate] ,[StartTime] ,[Status] ,[EndTime] ,[ElapsedTime],[Message] \
         		from (SELECT ''CON'' AS [DRSystem], ''R'' AS [ProcessCode] , ROW_NUMBER() OVER (PARTITION BY [sJOB].[job_id] ORDER BY [run_date], [run_time] ) AS [Seq] \
        , (select \
        ...
        DATEADD(dd,-1,getdate()) ) A ORDER BY [StartTime] \')" %(v_job_id)
